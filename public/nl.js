...
(function (global) {
  ...
  function parse(input, opts) {
    ...
    /* ---------- due date ---------- */
    var dd = null;
    var rel = /\b(?:due\s+|deadline\s+)?(?:is\s+|falls\s+)?(the day after tomorrow|day after tomorrow|tomorrow|tmrw|today|tonight|this weekend|next weekend|next week)\b(?! (\d|am|pm|o'))/i.exec(rest);
    if (rel) {
    ...
    if (dd && day0(dd) < today && !out.recurrence) { dd = addDays(day0(dd), 7); notes.push('that date was in the past — moved a week forward.'); }
    if (dd) {
